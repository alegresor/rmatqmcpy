...
def opt_weights_sum_1(x, kernel, dims=2, eps=1e-12, return_wce2_p_kmean=True):
    ...
    Args:
        ...
        return_wce2_p_kmean (bool): If True, return 
    ...
    Examples:
        ...
        >>> x.shape
        torch.Size([3, 7, 7])
        >>> w,wce2_p_kmean = opt_weights_sum_1(x,kernel)
        >>> w.shape 
        torch.Size([3])
        >>> w.sum() 
        tensor(1.)
        ...
        >>> xfull.shape 
        torch.Size([2, 3, 7, 7])
        >>> w_full,wce2_p_kmean_full = opt_weights_sum_1(xfull,kernel)
        >>> w_full.shape 
        torch.Size([2, 3])
        >>> w_full.sum(-1) 
        tensor([1., 1.])
    ...
    if return_wce2_p_kmean:
        ...
        return w,wce_squared_plus_kernel_integral
    else:
        return w
