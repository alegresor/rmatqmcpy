...
class KernelPolyFlag(object):
   ...
   Examples:
        ...
        >>> delta = delta/torch.linalg.norm(delta)
        >>> kernel = KernelPolyFlag(delta,r,c)
        >>> kernel.tau 
        ...
        >>> x = torch.einsum("...ij,...j,...kj->...ik",qx,delta,qx)
        >>> qy = tf_coe_qr(torch.rand((2**18,n**2),generator=rng))
        >>> y = torch.einsum("...ij,...j,...kj->...ik",qy,delta,qy)
        >>> x.shape
        torch.Size([8, 7, 7])
        >>> y.shape
        ...
        tensor(0.0002)
    """
    def __init__(self, delta, r, c):
        self.delta = delta 
        self.r = r
        self.c = c
        self.tau = compute_tau_int_traceXY_topow_r(self.delta,self.r)
        jvec = torch.arange(self.r+1,device=self.delta.device)
        rt = jvec[-1]
...
class KernelMaternChordal(object):
    ...
    Examples:
        ...
        >>> delta = torch.arange(1,8,dtype=float)
        >>> delta = delta/torch.linalg.norm(delta)
        >>> kernel = KernelMaternChordal(delta,nu=1/2)
    ...
    def __init__(self, delta, nu, sigma2=1, rho=1):
        self.delta = delta 
        self.nu = nu
        self.sigma2 = sigma2 
        self.rho = rho 
        self.tr_lam2 = (self.delta**2).sum()
        assert self.nu in self.SUPPORTED_NU, "nu should be in %s"%str(self.SUPPORTED_NU)
        if self.nu==1/2: 
            self.c = torch.tensor([1],dtype=self.delta.real.dtype,device=self.delta.device)
            self.nu_str = r"$\nu = 1/2$"
        elif self.nu==3/2:
            self.c = torch.tensor([1,np.sqrt(3)],dtype=self.delta.real.dtype,device=self.delta.device)
            self.nu_str = r"$\nu = 3/2$"
        elif self.nu==5/2:
            self.c = torch.tensor([1,np.sqrt(5),5/3],dtype=self.delta.real.dtype,device=self.delta.device)
            self.nu_str = r"$\nu = 5/2$"
        elif self.nu==7/2:
            self.c = torch.tensor([1,np.sqrt(7),14/5,7*np.sqrt(7)/15],dtype=self.delta.real.dtype,device=self.delta.device)
            self.nu_str = r"$\nu = 7/2$"
        elif self.nu==9/2:
            self.c = torch.tensor([1,3,27/7,18/7,27/35],dtype=self.delta.real.dtype,device=self.delta.device)
            self.nu_str = r"$\nu = 9/2$"
        elif self.nu==11/2:
            self.c = torch.tensor([1,np.sqrt(11),44/9,11*np.sqrt(11)/9,121/63,121*np.sqrt(11)/945],dtype=self.delta.real.dtype,device=self.delta.device)
            self.nu_str = r"$\nu = 11/2$"
        elif self.nu==13/2:
            self.c = torch.tensor([1,np.sqrt(13),65/11,52*np.sqrt(13)/33,338/99,169*np.sqrt(13)/495,2197/10395],dtype=self.delta.real.dtype,device=self.delta.device)
            self.nu_str = r"$\nu = 13/2$"
        elif self.nu==np.inf:
            self.nu_str = r"$\nu = \infty$"
    ...
    def __call__(self, x, y):
        ...
        if self.nu==np.inf:
            ...
            poly_term = (self.c*rscaled[...,None]**torch.arange(len(self.c),dtype=self.delta.real.dtype)).sum(-1)
